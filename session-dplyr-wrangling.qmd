...
:::incremental
- For each of the organisations `group_by()` or in `summarise()` with `.by = `
- In `summarise()` add sums for `total_beds` and `total_occupancy`. 
- `mutate()` the new data frame to create a percentage using the totals (occ / beds) 
